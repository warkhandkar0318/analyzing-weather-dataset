...
def clear(df,col,val):
    ...
    """
    value_counts=len(df[df[col]==val])
    return value_counts
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance=df[df[col1]>val1]
    instance=instance[instance[col2]==val2]
    return instance
...
def convert(df,celsius):
    ...
    """
    converted_temp=df[celsius].apply(lambda x:(9/5)*(x+32))
...
agg_values_ina_month=agg_values_ina_month(weather,'Date/Time','Dew Point Temp (C)', 'mean')
print(agg_values_ina_month)
...
print(convert(weather,'Dew Point Temp (C)'))
